...
def tracking_blink(
    ...
    while len(blinks) < size or state != IDLE:
        ...
        roi = cv2.cvtColor(
            frame[roi_y : roi_y + roi_h, roi_x : roi_x + roi_w], cv2.COLOR_RGB2GRAY
        )
        if (roi == prev_roi).all():
            continue
        ...
        if 0.01 < match < threshold:
            ...
            if state == IDLE:
                ...
                print(
                    f"Adv Since Last: {round((time_counter - prev_time)/1.018)} "
        ...
        if tk_window is None:
            ...
            if tk_window.config_json["display_percent"] != 100:
                ...
                frame = cv2.resize(
                    ...
                    (
                        round(
                            frame_width * tk_window.config_json["display_percent"] / 100
                        ),
                        round(
                            frame_height
                            * tk_window.config_json["display_percent"]
                            / 100
                        ),
                    ),
                )
    ...
    while len(blinks) < size or pl_state != IDLE:
        ...
        if pk_state == IDLE:
            ...
            res = cv2.matchTemplate(
                roi, pkimg, cv2.TM_CCORR_NORMED
            )  # CCORR might be better?
            _, match, _, _ = cv2.minMaxLoc(res)
            cv2.imshow("pk", roi)
            cv2.waitKey(1)
            if 0.4 < match < pkth:
...
def recov(blinks: List[int], rawintervals: List[int], npc: int = 0) -> Xorshift:
    ...
    if npc == 0:
        expected_blinks = [
            r & 0xF
            for r in prng.get_next_rand_sequence(advanced_frame)
            if r & 0b1110 == 0
        ]
        ...
        for distance in range(npc + 1):
            expected_blinks = [
                r & 0xF
                for r in prng.get_next_rand_sequence(advanced_frame * npc)[
                    distance :: npc + 1
                ]
                if r & 0b1110 == 0
            ]
...
def reidentiy_by_blinks(
    ...
    for distance in range(1 + npc):
        ...
        rands = [
            (i, r & 0xF)
            for i, r in list(
                enumerate(identify_rng.get_next_rand_sequence(search_max))
            )[distance :: 1 + npc]
        ]
...
def reidentiy_by_intervals_noisy(
    rng: Xorshift, rawintervals: List[int], search_max=10**5, search_min=0
) -> Xorshift:
    ...
    blink_rands = [
        int((r & 0b1110) == 0) for r in temp_rng.get_next_rand_sequence(search_max)
    ]
...
def recov_by_munchlax(rawintervals: List[float]) -> Xorshift:
    ...
    # validation check
    expected_intervals = [
        randrange(r, 100, 370) / 30 for r in prng.get_next_rand_sequence(advances)
    ]
