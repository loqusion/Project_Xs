...
def reidentify():
    ...
    pkroi = (805, 475, 20, 30)
    blinks, observed_intervals, offset_time = rngtool.simultaneous_tracking(
        plimg, plroi, pkimg, pkroi, pkth=0.999185, size=8
    )

    reidentified_rng = rngtool.reidentiy_by_intervals(
        Xorshift(*state), observed_intervals, threshold=2, search_max=10**5
    )
...
def reidentifyInSecretBase():
    ...
    if player_eye is None:
        print("path is wrong")
        return
    blinks, observed_intervals, offset_time = rngtool.tracking_blink(
        player_eye, 870, 680, 85, 90, size=7
    )
    reidentified_rng = rngtool.reidentiy_by_intervals(
        Xorshift(*state), observed_intervals, npc=0
    )
    if reidentified_rng is None:
        print("couldn't reidentify state.")
        return
...
def cresselia_timeline():
    ...
    pkroi = (805, 475, 20, 30)
    blinks, observed_intervals, offset_time = rngtool.simultaneous_tracking(
        plimg, plroi, pkimg, pkroi, pkth=0.999185, size=4
    )
